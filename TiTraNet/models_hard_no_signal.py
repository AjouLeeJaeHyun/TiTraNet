...
class VehicleEncoder(nn.Module):
    def __init__(self, agent_type_dim=4, embedding_dim=64, h_dim=64, num_layers=1, dropout=0.0):
...
# Cross-attention-based router
class CrossAttentionRouter(nn.Module):
    def __init__(self, query_dim, context_dim, hidden_dim=128, top_k=4):
    ...
    def forward(self, x, context):
        ...
        returns: masked x with only top-k routed elements per batch
        """
        seq_len, batch_size, query_dim = x.size()

        # Project queries and context
        query = self.query_proj(x)  # (seq_len, batch, hidden_dim)
        key = self.context_proj(context.squeeze(-1)).unsqueeze(0)  # Ensure shape is (1, batch, hidden_dim)

        # Compute attention score (dot product)
        scores = torch.einsum('sbd,bd->sb', query, key.squeeze(0))  # (seq_len, batch)

        # Get top-k per batch
        topk_scores, topk_indices = torch.topk(scores, self.top_k, dim=0)  # (top_k, batch)
        mask = torch.zeros_like(scores)
        mask.scatter_(0, topk_indices, 1.0)

        # Expand mask to match feature dim
        mask = mask.unsqueeze(2).expand(-1, -1, query_dim)  # (seq_len, batch, query_dim)
        x_masked = x * mask
        return x_masked
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        self.vehicle_encoder = VehicleEncoder(4, embedding_dim, 64)
        ...
        # 💡 Routing module 추가
        self.expected_feature_dim = 256
        self.routing = CrossAttentionRouter(query_dim=self.expected_feature_dim, context_dim=4, top_k=4)

        # decoder에 맞춰서 projection
        self.memory_projection = nn.Linear(self.expected_feature_dim, self.embedding_dim)

    def forward(self, obs_traj, obs_traj_rel, seq_start_end, vx, vy, ax, ay, agent_type, size):
        ...
        # traffic_encoding and its shape expansion removed
        combined_encoding = torch.cat([final_encoder_h, vehicle_encoding, state_encoding], dim=2)

        # Define context vector from last timestep of vx, vy, ax, ay
        device = vx.device
        vx_last = vx[-1].unsqueeze(1)
        vy_last = vy[-1].unsqueeze(1)
        ax_last = ax[-1].unsqueeze(1)
        ay_last = ay[-1].unsqueeze(1)
        context = torch.cat([vx_last, vy_last, ax_last, ay_last], dim=1)
