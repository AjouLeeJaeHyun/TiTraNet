...
class TrajectoryDataset(Dataset):
        ...
        for path in all_files:
            ...
            for idx in range(0, num_sequences * self.skip + 1, skip):
                ...
                for _, ped_id in enumerate(vehs_in_curr_seq):
                    ...
                    curr_vel_size = np.transpose(curr_ped_seq_[:, 7:9])
                    curr_vel_agent_type=np.transpose(curr_ped_seq_[:,9])
                    curr_vel_traffic_state=np.transpose(curr_ped_seq_[:,10])
    ...
    def __getitem__(self, index):
        ...
        out = [
            ...
            self.vx[start:end, :],self.vy[start:end, :],self.ax[start:end, :],
            # self.ay[start:end, :],
