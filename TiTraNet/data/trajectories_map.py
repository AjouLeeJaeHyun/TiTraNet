...
def seq_collate(data):
     ...
     *map_image_list) = zip(*data)  # map_tensor는 optional하게 처리
    ...
    # 🗺️ map tensor 추가 (모든 샘플에 동일한 맵이라고 가정)
    if map_image_list and map_image_list[0] is not None:
        # 하나의 맵 이미지를 모든 배치에 공유 (Batch 단위 추가)
        # 안전하게 맵 추출
        map_tensor = map_image_list[0]
        if isinstance(map_tensor, tuple):
            map_tensor = map_tensor[0]
        map_tensor = map_tensor.unsqueeze(0).repeat(obs_traj.size(1), 1, 1, 1).float()
        out.append(map_tensor)
...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        min_ped=1, delim='\t', map_image_path=None
    ):
        """
        Args:
        ...
        if map_image_path is not None:
            ...
            self.map_tensor = transform(image).float()
    ...
    def __getitem__(self, index):
        ...
        if self.map_tensor is not None:
            out.append(self.map_tensor.clone())  # 🗺️ 모든 agent에 동일한 map을 붙여줌
