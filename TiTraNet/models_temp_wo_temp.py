...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        # 💡 Routing module (disabled for ablation)
        # self.expected_feature_dim = 320
        # self.routing = CrossAttentionRouter(query_dim=self.expected_feature_dim, context_dim=65)

        # decoder에 맞춰서 projection
        self.memory_projection = nn.Linear(320, self.embedding_dim)
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, vx, vy, ax, ay, agent_type, size, traffic_state):
        ...
        g_centripetal = g_centripetal.view(-1, 1)  # (batch, 1)
        ...
        # context 벡터 구성
        context = torch.cat([traffic_context, g_centripetal], dim=1)  # (batch, 2)

        # Skip routing module for ablation (no temperature-scaled attention)
