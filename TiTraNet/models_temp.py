...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, obs_traj):
        ...
        # Flatten and embed
        obs_traj_embedding = self.spatial_embedding(obs_traj.reshape(-1, 2))  # [batch*obs_len, embedding_dim]
        obs_traj_embedding = obs_traj_embedding.view(batch_size, obs_len, self.embedding_dim)  # [batch, obs_len, embedding_dim]

        # Transformer expects [seq_len, batch, embedding_dim]
        obs_traj_embedding = obs_traj_embedding.permute(1, 0, 2)  # [obs_len, batch, embedding_dim]
...
class TransformerDecoder(nn.Module):
    def __init__(self, seq_len, embedding_dim=64, h_dim=128, mlp_dim=1024, num_layers=1, dropout=0.0):
        ...
        # Transformer Decoder 사용 (ensure batch_first=True)
    ...
    def forward(self, last_pos, last_pos_rel, memory, seq_start_end, vx, vy):
        ...
        # (batch, 2) → (batch, 1, embedding_dim)
        decoder_input = self.spatial_embedding(last_pos_rel).unsqueeze(1)

        # zero-input decoding: repeat previous embedding
        decoder_inputs = decoder_input.repeat(1, self.seq_len, 1)  # [B, pred_len, D]

        # generate causal mask
        tgt_mask = nn.Transformer.generate_square_subsequent_mask(self.seq_len).to(memory.device)  # [T, T]

        # ensure memory shape is [B, T, D]
        if memory.size(0) != batch:
        ...
        # output → [pred_len, batch, 2]
        pred_traj_fake_rel = self.hidden2pos(decoder_output).permute(1, 0, 2)
...
class CrossAttentionRouter(nn.Module):
    def __init__(self, query_dim, context_dim, hidden_dim=128):
        ...
        # context_dim should be 128 ([batch, 128])
        self.context_proj = nn.Linear(128, hidden_dim)
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        self.routing = CrossAttentionRouter(query_dim=self.expected_feature_dim, context_dim=128)

        # decoder에 맞춰서 projection
        self.memory_projection = nn.Linear(self.expected_feature_dim, self.embedding_dim)

        # traffic context projection
        self.traffic_proj = nn.Linear(64, 64)
        # Centripetal projection to match context_dim
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, vx, vy, ax, ay, agent_type, size, traffic_state):
        ...
        #print(f"final_encoder_h shape: {final_encoder_h.shape}")
        vehicle_encoding = self.vehicle_encoder(agent_type, size).permute(1, 0, 2)
        #print(f"vehicle_encoding shape (before fix): {vehicle_encoding.shape}")
        # Fix vehicle_encoding to match temporal dimension
        if vehicle_encoding.dim() == 2:
            vehicle_encoding = vehicle_encoding.unsqueeze(0).repeat(final_encoder_h.size(0), 1, 1)  # Expand to (T, B, D)
        #print(f"vehicle_encoding shape (after fix): {vehicle_encoding.shape}")
        state_encoding = self.state_encoder(torch.cat([vx, vy, ax, ay], dim=2)).permute(1, 0, 2)
        traffic_encoding = self.traffic_encoder(traffic_state).permute(1, 0, 2)
        ...
        # Define context vector: use traffic encoding + cross-product of velocity and acceleration as context
        vx_last = vx[-1]
        vy_last = vy[-1]
        ax_last = ax[-1]
        ay_last = ay[-1]
        g_centripetal = vx_last * ay_last - vy_last * ax_last  # (batch,)
        # Reshape to (batch, 1)
        if g_centripetal.dim() == 1:
            g_centripetal = g_centripetal.view(-1, 1)
        elif g_centripetal.dim() > 1 and g_centripetal.size(1) != 1:
            g_centripetal = g_centripetal.mean(dim=1, keepdim=True)
        ...
        # Project traffic_context and centripetal to (1, context_dim) and concatenate for context (1, 128)
        g_centripetal_pooled = g_centripetal.mean(dim=0, keepdim=True)  # (1, 1)
        g_centripetal_projected = self.centripetal_proj(g_centripetal_pooled)  # (1, context_dim)
        # Handle shape mismatch for context
        if g_centripetal_projected.size(0) != traffic_context.size(0):
            g_centripetal_projected = g_centripetal_projected.expand(traffic_context.size(0), -1)
        context = torch.cat([traffic_context, g_centripetal_projected], dim=1)
