...
class StateEncoder(nn.Module):
    def __init__(self, embedding_dim=64, h_dim=64, num_layers=1, dropout=0.0):
        ...
        self.state_layer = nn.Linear(3, embedding_dim)  # (vx, vy, ax, ay → embedding_dim)
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, vx, vy, ax, agent_type, size, traffic_state):
...
# 새로 만든 def forward(pooling type 제거됨)
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, vx, vy, ax, agent_type, size, traffic_state, user_noise=None):
        batch = obs_traj_rel.size(1)
        ...
        state_encoding = self.state_encoder(torch.cat([vx, vy, ax], dim=2))
