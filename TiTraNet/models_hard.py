...
class CrossAttentionRouter(nn.Module):
    ...
    def forward(self, x, context):
        ...
        key = self.context_proj(context)  # shape: (batch, hidden_dim)
        key = key.unsqueeze(0)
        key = key.expand(x.size(0), key.size(1), key.size(2))  # shape: (seq_len, batch, hidden_dim)

        # Compute attention score (dot product): (seq_len, batch, hidden_dim) · (seq_len, batch, hidden_dim) -> (seq_len, batch)
        scores = torch.einsum('sbd,sbd->sb', query, key)  # (seq_len, batch)
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        self.routing = CrossAttentionRouter(query_dim=self.expected_feature_dim, context_dim=2, top_k=4)
    ...
    def forward(self, obs_traj, obs_traj_rel, seq_start_end, vx, vy, ax, ay, agent_type, size, traffic_state):
        ...
        g_centripetal = g_centripetal.unsqueeze(1)  # (batch, 1)
        # Use traffic encoding + cross-product of velocity and acceleration as context
        context = torch.cat([traffic_encoding[-1].mean(dim=1), g_centripetal], dim=1)  # (batch, 2)
