...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        self.map_encoder = CNNMapEncoder(output_dim=map_feature_dim)
    ...
    # 안쓰는 forward 함수, 아래에 새로운 forward 함수가 있음
    # def forward(self, obs_traj, obs_traj_rel, seq_start_end,
    #             vx, vy, ax, ay, agent_type, size, traffic_state, map_image):
    ...
    #     map_feature = self.map_encoder(map_image)
    
    #     vehicle_encoding = vehicle_encoding.expand(self.obs_len, -1, -1)
    #     state_encoding = state_encoding.expand(self.obs_len, -1, -1)
    #     traffic_encoding = traffic_encoding.expand(self.obs_len, -1, -1)
    #     map_feature = map_feature.unsqueeze(0).expand(self.obs_len, -1, -1)
    ...
    #     total_feature_dim = combined_encoding.shape[2]
    
    #     mlp_decoder_context_input = combined_encoding.view(self.obs_len, batch, total_feature_dim)
    
    #     if not hasattr(self, 'memory_projection'):
    #         self.memory_projection = nn.Linear(total_feature_dim, self.embedding_dim).to(combined_encoding.device)
    #     memory = self.memory_projection(mlp_decoder_context_input)
...
# 새로 만든 def forward(pooling type 제거됨)
    def forward(self, obs_traj, obs_traj_rel, seq_start_end,
        ...
        # 👉 1. 기본 인코딩
        final_encoder_h = self.encoder(obs_traj_rel)  # (obs_len, batch, embed)
        vehicle_encoding = self.vehicle_encoder(agent_type, size)  # (1, B, H)
        state_encoding = self.state_encoder(torch.cat([vx, vy, ax, ay], dim=2))  # (1, B, H)
        traffic_encoding = self.traffic_encoder(traffic_state)  # (1, B, H)

        # 👉 2. 지도 인코딩
        map_encoding = self.map_encoder(map_image)  # (B, map_feat)
        map_encoding = map_encoding.unsqueeze(0).expand(self.obs_len, -1, -1)
        ...
        combined_encoding = torch.cat([
            ...
            traffic_encoding,
            map_encoding
        ], dim=2)  # (obs_len, batch, total_dim)

        total_feature_dim = combined_encoding.shape[2]

        # 👉 5. memory_projection 안전하게 처리
        if not hasattr(self, 'memory_projection'):
            # ⚠️ 최초 호출 시만 projection layer 만들고 붙임 (lazy init)
            self.memory_projection = nn.Linear(total_feature_dim, self.embedding_dim).to(combined_encoding.device)
...
class TrajectoryDiscriminator(nn.Module):
    ...
    def forward(self, traj, traj_rel, map_data, seq_start_end=None):
        """
        Inputs:
        - traj_rel: Tensor (obs_len + pred_len, batch, 2)
        - map_data: Tensor (batch, C, H, W)
        """
        batch = traj_rel.shape[1]

        # Trajectory embedding
        traj_rel_embedded = self.spatial_embedding(traj_rel)
        traj_rel_embedded = traj_rel_embedded + self.positional_encoding

        # Map feature (shared for all time steps)
        map_data = map_data.float()
        map_feature = self.map_encoder(map_data)  # (batch, map_feature_dim)
        map_feature = map_feature.unsqueeze(0).expand(self.seq_len, -1, -1)  # (seq_len, batch, map_feature_dim)

        # Combine trajectory + map
        combined_input = torch.cat([traj_rel_embedded, map_feature], dim=2)  # (seq_len, batch, embed+map)

        # Transformer Encoder
        final_h = self.encoder(combined_input)

        # Classify based on last time step
        classifier_input = final_h[-1, :, :]
